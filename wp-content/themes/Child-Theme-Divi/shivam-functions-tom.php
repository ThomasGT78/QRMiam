...
/**
 * Récupère les infos d'un client dans la table cn_user_doce en fonction de l'id
...
function upload_document() {	// end at line 1160
	...
	if (get_current_user_id()) {	// end at line 1128
		...
		$cn_user_doc = get_user_documents_By_Id($cn_user_by_user->ID);
		...
		$user_etablissement_name = $cn_all_user_meta['user_registration_input_box_1591092586428'][0];
		...
		if ($_GET['myprofile']=='usename') {
			echo $url = $site_url.'/upload/?username/' . $cn_user->user_login;
	?>
	...
	**************************************************************************************************/
		...
		?>
			...
			if ($_FILES['cn_image']) { 
				...
				for($i=0;$i<$countfiles;$i++) {
					...
					$ext1 = explode('.', basename($_FILES['cn_image']['name'][$i]));
		...
		if (isset($_POST['cn_delete'])) {
			...
			$SQL = "DELETE FROM `".$cn_user_doce."` WHERE `id`= $cn_img_id";
		...
		if (isset($_POST['cn_update'])) {
			...
			if($_POST['cn_etablissement_name']) {
				$new_etablissement_name = $_POST['cn_etablissement_name'];
			...
			if($_POST['cn_etablissement_color']) {
				...
				$user_etablissement_color = $new_etablissement_color;
			} else {
			...
			$response_etablissement_name = iUpdateArrayMultiConds(
				$usermeta, 
				$result_etablissement_name, 
				array(
					'user_id' => get_current_user_id(),
					'meta_key' => 'user_registration_input_box_1591092586428'
				)
			);	
			
			$response_etablissement_color = iUpdateArrayMultiConds(
				$users, 
	...
	<?php
		...
		$cn_user_doce_details = get_user_documents_By_Id(get_current_user_id());
	...
	<div class="user-registration ur-frontend-form ur-frontend-form--flat">
		...
		<form  method="post" accept-charset="utf-8" enctype="multipart/form-data">
			<div class="cn_title">
				...
				<h2 class="text-center" style="color:<?php echo $user_etablissement_color; ?>" >
					<?php echo $user_etablissement_name; ?>
				</h2>
				<!-- <h2 style="color:<?php echo $user_etablissement_color; ?>">Test Couleur</h2> -->

				<!-- Nom d'établissement -->
				<div style="overflow: hidden; margin-bottom: 5px;">
					<label>Nom de l'établissement </label>	
					<input style="!important;" type="text" name="cn_etablissement_name" value="<?php echo $user_etablissement_name; ?>">	
				</div>
				
				<!-- Couleur d'établissement -->
				<div style="overflow: hidden; margin-bottom: 5px;">
					<label>Couleur du nom de l'établissement </label>	
					<input style="!important;" type="color" name="cn_etablissement_color" value="<?php echo $user_etablissement_color; ?>">	
				</div>
				<div>
			...
			// USER PREMIUM
				...
				?>
						...
						<div style="overflow: hidden; margin-bottom: 5px;">
							...
							<input type="file" name="cn_image[]" class="cn-form-control cn_image" placeholder="Image"  accept="application/pdf,image/*" required="required" multiple>
						</div>
						<br>
				<?php
					}  // if ($new_date >= time())
			...
			// USER LIGHT
				...
				?>
					<div style="overflow: hidden; margin-bottom: 5px;">
						...
						<input type="file" name="cn_image[]" class="cn-form-control cn_image" placeholder="Image"  accept="image/*" required="required" multiple>
					</div>
			<?php
				}
			?>
			...
			if ($new_date >= time()) {
				...
				<div class="Uploaded_doc">
					...
					<?php
						...
						} // if ($cn_user_banner_details) => has banner uploaded
		...
		}  // if ($completed == 'completed') => Premium
			?>
		...
		<div class="Uploaded_doc">
			...
			<p>
				...
				<a href="#" title="En cours de développement">Statistiques</a>
			</p>
			<!-- END Statistiques sur les vues -->
			...
			if ($cn_user_doce_details) {
				...
				foreach ($cn_user_doce_details as $value) {
					$fileExtension = explode('.', $value['file']);
			?>
					<!------------------------------------
					-	LIST DOCUMENT ITEMS and UPLOAD	-- 
					------------------------------------->
					<hr>
					<div class="et_pb_row et_pb_row_1 et_pb_row_4col">
						<div class="et_pb_column et_pb_column_1_3 et_pb_column_1  et_pb_css_mix_blend_mode_passthrough">
							...
							if ($fileExtension[2] == 'pdf') {
								...
								<a target="_blank" href="<?php echo $value['file']; ?>?#zoom=0">
									...
									<p style="text-align: center; color:<?php echo $value['color']; ?>; font-size: 17px;">
							...
							?>
								<a href="javascript: void(0);" onclick="open_img('<?php echo $value['file']; ?>');">
									<img src="<?php echo $value['file']; ?>" alt="" style="width: 100%;">
...
**************************************************************************************************-->
	...
	} // end if (get_current_user_id() (ligne 437)
	else {
...
function cn_public_page($site_url){ // Fin ligne 1484
...
**************************************************************************************************-->
			...
			<div class="Uploaded_doc">
				...
				<h2 style="color:<?php echo $etablissement_color[0]['etablissement_color']; ?>" >
					<center>
						<?php echo $cn_all_user_meta['user_registration_input_box_1591092586428'][0]; ?>
					</center>
				</h2>
				<br>
									...
									<div class="et_pb_gallery_image landscape">
										...
										?>
											<a data-fancybox="gallery" data-options='{"buttons": ["zoom","thumbs","close"]}' href="<?php echo $value['file']; ?>">
												...
												<p style="   margin-top: 17px; text-align: center; color:<?php echo $value['color']; ?>; font-size: 17px;">
													<?php echo htmlspecialchars($value['title']); ?>
												</p>
											</a>
...
function membership_product($site_url) { // en at line 1797
	ob_start();
  	$args = array(
		'posts_per_page' => -1, 
		'post_type' => 'product',
...
function gen_qrcode($site_url) {

    $user = wp_get_current_user();
	//    return $user->display_name;
	//   if($user->display_name != ""){
	if (get_current_user_id()) {
    	$qrcode_url = $site_url."/Profile/?username=" . $user->display_name;
	?>
	<div style="text-align: center;">
		<h5 style="text-align: center;">Voici votre QRCode personnel à télécharger, clic droit sur PC ou appui long sur smartphone</h5>
	<?php
...
function stats_page() {
	...
	<form action="" method="POST">
		...
		<?php 
		if(get_current_user_id()==64 OR get_current_user_id()==1 OR get_current_user_id()==30) { 
